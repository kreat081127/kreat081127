...
font_name = pg.font.match_font('arial') # переменная
heart = pg.image.load(path.join(img_dir, "heart.png")).convert() # картинка сердечек
player_img = pg.image.load(path.join(img_dir, "Player_car.png")).convert() # картинка машины игрока
car_png = []
car_list = ["G_car.png", "W_car.png", "Gr_car.png"]  # картинка машин(врагов)

for img in car_list: # лист хранения пнг картинок
...
def user_name(surf, text, x, y, size=FONT_SIZE): # имя пользователя
    font = pg.font.Font(font_name, size)
...
class Player(pg.sprite.Sprite):  # daet Bosmoжность что-то делать в объекте
    ...
    def update(self):  # спользуется для добавления новых элементов
        self.speedx = 0
        self.speedy = 0
...
class Mob(pg.sprite.Sprite):  # помагает создать враж машину
    def __init__(self):
        pg.sprite.Sprite.__init__(self)
...
class Heart(pg.sprite.Sprite): # хп(здоровье)
...
def update_game_screen(name: str, score: int): # создание линний
    screen.fill(DARK_GREY)
    for i in range(6):  # колл-во линий
        pg.draw.line(screen, WHITE, [WIDTH / 5 * i, - HEIGHT], [WIDTH / 5 * i, HEIGHT], 4) # рисовка линии
    all_sprites.draw(screen)
    draw_text(screen, str(score), (WIDTH / 2) + 30, 10) # осознает текст
    user_name(screen, str(name), (WIDTH / 3) + 30, 10) # имя пользователя
    pg.display.flip()
...
def score_game(): # колл-во очков набраных игроком
    return (f'Ваш результат : {db.get(name)}')


def top_gamers(): # выводит все лучшие данные
    offset = 0
...
game_over_loop = True # переменная включающяя игру
while game_over_loop:
    ...
    screen.fill(BLACK)
    draw_text(screen, 'Game Over', WIDTH // 2, HEIGHT - 450) # надпись звключения игры
    draw_text(screen, score_game(), WIDTH // 2, HEIGHT // 2) # колл-во очков набраных игроком
    draw_text(screen, 'Best scores:', WIDTH // 2, HEIGHT - 250) # лучший резулятат игрока
    top_gamers()
    pg.display.flip()
